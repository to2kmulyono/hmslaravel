...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Form Edit Rekam Medis</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('dokter.rekam-medis.update', $rm->id) }}" method="POST">
            ...
            <div class="form-group">
                <label>Pasien</label>
                <input type="text" class="form-control" value="{{ $rm->pasien->nama_user ?? 'Unknown' }} ({{ $rm->pasien->username ?? '-' }})" readonly>
                <small class="text-muted">Pasien tidak dapat diubah setelah rekam medis dibuat.</small>
            </div>

            <div class="form-group">
                <label for="keluhan_utama">Keluhan Utama <span class="text-danger">*</span></label>
                <textarea name="keluhan_utama" id="keluhan_utama" rows="3" class="form-control @error('keluhan_utama') is-invalid @enderror" required>{{ old('keluhan_utama', $rm->keluhan_utama) }}</textarea>
                @error('keluhan_utama')
            ...
            <div class="form-group">
                ...
                <textarea name="diagnosa" id="diagnosa" rows="3" class="form-control @error('diagnosa') is-invalid @enderror" required>{{ old('diagnosa', $rm->diagnosa) }}</textarea>
                @error('diagnosa')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="tindakan" id="tindakan" rows="3" class="form-control @error('tindakan') is-invalid @enderror">{{ old('tindakan', $rm->tindakan) }}</textarea>
                @error('tindakan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="resep_obat" id="resep_obat" rows="3" class="form-control @error('resep_obat') is-invalid @enderror">{{ old('resep_obat', $rm->resep_obat) }}</textarea>
                @error('resep_obat')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
